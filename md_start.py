def start(self):
        ...
        ['Код сотрудника'  ,'Категория'  ,'ФИО сотрудника'  ,'Дата приёма'  ,'Дата увольнения'  ,'Число авторских свидетельств'  ,'Наименование аутстафф-компании'  ,'Возможность занимания руководящей должности'],
        ['Код оборудования','Название оборудования', 'Категория',],
        ['Код проекта','Материалы проекта', 'Оборудование', 'Руководитель','Категория',],
        ['Код заказчика', 'ФИО заказчика','Паспортные данные заказчика','Адрес заказчика', 'Номер телефона заказчика'],
        ...
        '(project_materials,equipment_id,head_employee_id,category_id)',
        ...
        self.ids=['category_id','department_id','employee_id','equipment_id','project_id','customer_id','contract_id']
	...
	"select count(*) as 'Кол-во заказов', (select customer_full_name from Customer where Customer.customer_id = Contract.customer_id) as 'ФИО заказчика' from Contract group by Contract.customer_id order by customer_id desc"
        ...
        self.qtitles=[
         ['Код проекта','Материалы проекта', 'Код оборудования', 'Код руководителя','Категория',],
         ['Название категории'],
         ['ФИО сотрудника'],
